...
static irqreturn_t net_interrupt(int irq, void *dev_id)
{
	struct net_device *dev = dev_id; // занятно как void* присваивается указателю на структуру, приведения типа не требуется, так наверное только с этими указателями
	struct net_local *np;
	int ioaddr, status;
	int handled = 0;
	...
	if (status & RX_INTR) {
        // да, получен пакет, дернуть net_rx, это внутренняя функция драйвера
		/* Got a packet(s). */
		net_rx(dev);
	}
#if TX_RING
...
net_rx(struct net_device *dev)
{
	...
	do {
		...
		if (status & 0x40) {	/* There was an error. */
			...
			skb = dev_alloc_skb(pkt_len); // а вот интересный вызов, создаем буферный сокет, куда будем сохранять принятые данные
			if (skb == NULL) {
                // вообще говоря странно, разве этот пакет не остается в памяти?
                // и не будет подхвачен потом снова? мне кажется нужно выдать команду на сброс пакета, а может быть пакет будет прочитан в следующем проходе, после освобождения памяти
			...
			// не описана одна очень важная вещь, это протокол, например ETH_P_IP
			// и обязательно завернуть в htons, хотя я не понимаю зачем это нужно, ведь дальше же это не пойдет
			skb->protocol = htons(ETH_P_IP);
			// КРУТО, ЭТО ОЧЕНЬ ВАЖНАЯ СТРОКА, указывает откуда, с какого устройства пришел пакет
			...
			netif_rx(skb); // передаем сокетный буфер на уровень выше
			dev->last_rx = jiffies; // видимо когда пришел пакет
			// оказывается есть еще jiffies_64, но она не атомарная
			lp->stats.rx_packets++;
			lp->stats.rx_bytes += pkt_len;
