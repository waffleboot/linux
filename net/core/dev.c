...
// когда приходит пакет, то драйвер создает сокетный буфер, кладет туда все что пришло с устройства
// и закидывает задачу на обработку, выставляе флаг программного прерывания
// я где-то читал, что программное прерывание обрабатывается в конце обычного прерывания, но надо разбираться в этом
